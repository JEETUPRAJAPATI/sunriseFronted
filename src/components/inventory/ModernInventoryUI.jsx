...
export default function ModernInventoryUI() {
  ...
  const { data: itemsData, isLoading: itemsLoading } = useQuery({
    queryKey: [`${import.meta.env.VITE_API_URL}/api/items`],
  });

  const { data: stats, isLoading: statsLoading } = useQuery({
    queryKey: [`${import.meta.env.VITE_API_URL}/api/inventory/stats`],
  });

  const { data: categoriesData } = useQuery({
    queryKey: [`${import.meta.env.VITE_API_URL}/api/categories`],
  });
  ...
  const deleteItemMutation = useMutation({
    mutationFn: (id) => apiRequest('DELETE', `${import.meta.env.VITE_API_URL}/api/items/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries([`${import.meta.env.VITE_API_URL}/api/items`]);
      queryClient.invalidateQueries([`${import.meta.env.VITE_API_URL}/api/inventory/stats`]);
  ...
  const createItemMutation = useMutation({
    mutationFn: (data) => apiRequest('POST', `${import.meta.env.VITE_API_URL}/api/items`, data),
    onSuccess: () => {
      queryClient.invalidateQueries([`${import.meta.env.VITE_API_URL}/api/items`]);
      queryClient.invalidateQueries([`${import.meta.env.VITE_API_URL}/api/inventory/stats`]);
      setShowForm(false);
  ...
  const updateItemMutation = useMutation({
    mutationFn: ({ id, data }) => apiRequest('PUT', `${import.meta.env.VITE_API_URL}/api/items/${id}`, data),
    onSuccess: () => {
      queryClient.invalidateQueries([`${import.meta.env.VITE_API_URL}/api/items`]);
      queryClient.invalidateQueries([`${import.meta.env.VITE_API_URL}/api/inventory/stats`]);
      setShowForm(false);
