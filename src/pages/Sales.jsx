...
export default function Sales() {
  ...
  const { data: salesData, isLoading } = useQuery({
    queryKey: [`${import.meta.env.VITE_API_URL}/api/sales?page=${page}&limit=10&search=${search}&paymentStatus=${paymentStatus}`],
    enabled: true
  });
  ...
  const deleteSaleMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: [`${import.meta.env.VITE_API_URL}/api/sales`]
      });
      toast({
        title: "Success",
