...
export default function Accounts() {
  ...
  const { data: accountsData, isLoading: accountsLoading } = useQuery({
    queryKey: [`${import.meta.env.VITE_API_URL}/api/accounts?page=${page}&limit=10&search=${search}&accountType=${accountType}`],
    enabled: true
  });

  const { data: transactionsData, isLoading: transactionsLoading } = useQuery({
    queryKey: [`${import.meta.env.VITE_API_URL}/api/transactions?page=${page}&limit=10&search=${search}`],
    enabled: true
  });
  ...
  const deleteAccountMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: [`${import.meta.env.VITE_API_URL}/api/accounts`]
      });
      toast({
        title: "Success",
