...
export default function Dispatches() {
  ...
  const { data: dispatchesData, isLoading } = useQuery({
    queryKey: [`${import.meta.env.VITE_API_URL}/api/dispatches?page=${page}&limit=10&search=${search}&status=${status}`],
    enabled: true
  });
  ...
  const deleteDispatchMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [`${import.meta.env.VITE_API_URL}/api/dispatches`] });
      toast({
        title: "Success",
