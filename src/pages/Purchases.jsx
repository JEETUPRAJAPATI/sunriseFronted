...
export default function Purchases() {
  ...
  const { data: purchasesData, isLoading } = useQuery({
    queryKey: [`${import.meta.env.VITE_API_URL}/api/purchases?page=${page}&limit=10&search=${search}&status=${status}&paymentStatus=${paymentStatus}`],
    enabled: true
  });
  ...
  const deletePurchaseMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: [`${import.meta.env.VITE_API_URL}/api/purchases`]
      });
      toast({
        title: "Success",
