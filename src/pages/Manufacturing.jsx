...
export default function Manufacturing() {
  ...
  const { data: jobsData, isLoading } = useQuery({
    queryKey: [`${import.meta.env.VITE_API_URL}/api/manufacturing?page=${page}&limit=10&search=${search}&status=${status}`],
    enabled: true
  });
  ...
  const deleteJobMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: [`${import.meta.env.VITE_API_URL}/api/manufacturing`]
      });
      toast({
        title: "Success",
